...
export function computeWorldHash(world: WorldData): string {
  ...
  const terrainString = world.terrain
    .flat()
    .map((cell: TerrainCell) => `${cell.x},${cell.y}:${cell.elevation.toFixed(4)}:${cell.type}`)
    .join("|");
...
export function executeAction(world: WorldData, action: WorldAction, existingActions: WorldAction[]): ActionResult {
  // Guard against incomplete world data
  if (!world || !world.terrain || world.terrain.length === 0 || !world.gridSize) {
    return {
      success: false,
      message: "WORLD_NOT_READY",
      hash: "00000000",
    };
  }
  ...
  const existingAtLocation = existingActions.find((a) => a.gridX === gridX && a.gridY === gridY);
...
export function serializeActions(actions: WorldAction[]): string {
  ...
  return actions.map((a) => `${a.type}:${a.gridX}:${a.gridY}`).join(";");
}
...
export function generateReplayPath(world: WorldData, action?: WorldAction): ReplayFrame[] {
  ...
  // Guard against incomplete world data
  if (!world || !world.terrain || world.terrain.length === 0 || !world.spawnPoint || !world.plantedObject) {
    return frames;
  }
...
export function computeHashWithBreak(world: WorldData, breakType: "math_random" | "date" | "none"): string {
  if (breakType === "none") {
...
export function runDeterminismTest(world: WorldData, breakType: "math_random" | "date" | "none"): DeterminismTest {
