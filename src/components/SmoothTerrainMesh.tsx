...
if (cell) {
  ...
  const nearRiver =
    isRiver || !!left?.hasRiver || !!right?.hasRiver || !!up?.hasRiver || !!down?.hasRiver;
  ...
  if (nearRiver) {
    ...
    const centerFactor = Math.min(1, riverNeighbors / 2); // straight rivers still deep
    ...
    // ✅ IMPORTANT: clamp relative to *baseH* only (prevents "river ridge")
    // never allow raising above baseH - small carve
    const MIN_CARVE = isRiver ? 0.10 : 0.02;   // ensure visible cut
    const MAX_CARVE = isRiver ? 0.45 : 0.10;   // avoid insane trenches
