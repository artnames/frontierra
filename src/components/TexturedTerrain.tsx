...
// FIXED: Added missing 'riverbed' properties and relative carving logic.
...
export function TexturedTerrainMesh({
  ...
  const geometriesPerKind = useMemo(() => {
    ...
    // Fixes TS2741: record must include 'riverbed'
    ...
    // Relative Carving logic to match SmoothTerrainMesh
    ...
    for (let y = 0; y < size; y++) {
      ...
      for (let x = 0; x < size; x++) {
        ...
        // RELATIVE CARVING: Depth remains constant relative to slope altitude
    ...
    for (const kind of MATERIAL_KINDS) {
      ...
      for (const { x, y, cell, height } of cells) {
        ...
        const microVar = ((Math.sin(x * 12.9898 + y * 78.233 + world.seed * 0.1) * 43758.5453) % 1) * 0.15 - 0.075;
